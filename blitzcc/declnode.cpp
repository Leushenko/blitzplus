...
void FuncDeclNode::proto( DeclSeq *d,Environ *e ){
	...
	sem_type=d_new FuncType( t,decls.release(),false );
...
void FuncDeclNode::translate( Codegen *g ){
	...
	for( k=0;k<sem_env->decls->size();++k ){
		...
		if( d->kind!=DECL_PARAM || d->type!=Type::string_type ) continue;
		g->code( call( "__bbStrRetain",mem(local(d->offset)) ) );
	}
