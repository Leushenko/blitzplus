...
static std::vector<int> tmp_stack;
...
void StmtNode::pushTmps(){
	tmp_stack.push_back( tmp_strs );
}

void StmtNode::freeTmps( Codegen *g ){
	int n=tmp_strs-tmp_stack.back();
	tmp_stack.pop_back();
	if( !n ) return;
	g->code( call( "__bbStrTmpFree",iconst(n) ) );
	tmp_strs-=n;
}

TNode *StmtNode::freeTmps( TNode *t,Type *ty ){
	int n=tmp_strs-tmp_stack.back();
	tmp_stack.pop_back();
	if( !n ) return t;
	if( ty->floatType() ){
		t=fcall( "__bbFStrTmpFree",t,iconst(n) );
	}else{
		t=call( "__bbIStrTmpFree",t,iconst(n) );
	}
	tmp_strs-=n;
	return t;
}
...
void DimNode::translate( Codegen *g ){
	...
	g->i_data( et );
	g->i_data( exprs->size() );
	for( unsigned int k=0;k<exprs->size();++k ) g->i_data( 0 );
}
...
void IfNode::translate( Codegen *g ){
	if( ConstNode *c=expr->constNode() ){
		...
		pushTmps();
		TNode *t=expr->translate(g);
		g->code( jumpf( freeTmps(t,expr->sem_type),_else ) );
...
void WhileNode::translate( Codegen *g ){
	...
	if( ConstNode *c=expr->constNode() ){
		...
		pushTmps();
		TNode *t=expr->translate(g);
		g->code( jumpt( freeTmps(t,expr->sem_type),loop ) );
	}
	g->label( sem_brk );
}
...
void ForNode::translate( Codegen *g ){
	...
	g->code( jumpf( freeTmps(t,Type::int_type),loop ) );
...
void ForEachNode::translate( Codegen *g ){
	...
	t=call( objFirst,l,r );
	t=jumpf( freeTmps(t,Type::int_type),sem_brk );
	g->code( t );
	...
	pushTmps();
	t=call( objNext,var->translate( g ) );
	t=jumpt( freeTmps(t,Type::int_type),_loop );
...
void ReturnNode::translate( Codegen *g ){
	...
	pushTmps();
	TNode *t=expr->translate(g);
	if( expr->sem_type->stringType() ) t=call( "__bbStrRetain",t );
	t=freeTmps(t,expr->sem_type);
...
void SelectNode::translate( Codegen *g ){
	...
	for( unsigned int k=0;k<cases.size();++k ){
		...
		for( int j=0;j<c->exprs->size();++j ){
			...
			g->code( jumpt( freeTmps(t,Type::int_type),labs.back() ) );
		}
	}
...
void RepeatNode::translate( Codegen *g ){
	...
	if( ConstNode *c=expr ? expr->constNode() : 0 ){
		...
		if( expr ){
			...
			pushTmps();
			TNode *t=expr->translate(g);
			g->code( jumpf( freeTmps(t,expr->sem_type),loop ) );
		}else{
			g->code( jump( loop ) );
		}
	}
